...
export default function CitizenPortal() {
  ...
  useEffect(() => {
    if (user?.user_metadata?.full_name) {
      ...
      setReporterName(user.email.split('@')[0]);
    }
  }, [user]);
  ...
  if (!isLoggedIn) {
    return (
      <div style={{ padding: '60px 20px', maxWidth: '500px', margin: '0 auto', textAlign: 'center' }}>
        <div className="glass-panel-glow animate-slide-in" style={{ padding: '40px 30px', backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-lg)', display: 'flex', flexDirection: 'column', gap: '20px', alignItems: 'center' }}>
          <div style={{ padding: '16px', borderRadius: '50%', backgroundColor: 'var(--accent-glow)', color: 'var(--accent)', display: 'inline-flex' }}>
            <Send size={32} />
          </div>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: 0 }}>Authentication Required</h2>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', margin: 0, lineHeight: '1.5' }}>
            To prevent spam submissions and ensure accountability under the Digital India guidelines, citizen grievance registration requires a verified sign-in.
          </p>
          <button 
